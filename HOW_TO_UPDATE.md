# How to Update oxygen-patched-bcp47j from Upstream

**Upstream repository:** [https://github.com/rmraya/BCP47J](https://github.com/rmraya/BCP47J)
...
This project depends on **oxygen-patched-xmljava**; ensure that dependency is updated or compatible when updating BCP47J.
...
Set the following in `<properties>` (get version and build ID from `Constants.java` in `src/com/maxprograms/languages/`):

```xml
<properties>
  <!-- other properties -->
  <patched.groupId>com.maxprograms</patched.groupId>
  <patched.artifactId>bcp47j</patched.artifactId>
...
#### 3.3 Dependency on oxygen-patched-xmljava

If the upstream BCP47J bumps its dependency on XMLJava, align the Maven dependency version in `<dependencies>` with the **oxygen-patched-xmljava** version you are using in the workspace.
